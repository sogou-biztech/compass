...
public class ModShardDataSourceRouteStrategy implements ShardDataSourceRouteStrategy {
    ...
    public TableContext route(Object routeKey, ShardDataSourceContext shardDataSourceContext) {
        ...
        if (CollectionUtils.isEmpty(masterSlaveDataSourceIds)) {
            throw new IllegalArgumentException("ShardDataSource: [" + shardDataSourceContext.getShardDataSourceId()
                + "] masterSlaveDataSourceIds is empty");
        }
    ...
    @Override
    public List<TableContext> getMasterSlaveDataSourceTableContexts(String masterSlaveDataSourceId,
        ShardDataSourceContext shardDataSourceContext) {
        List<String> masterSlaveDataSourceIds = shardDataSourceContext.getMasterSlaveDataSourceIds();
        if (CollectionUtils.isEmpty(masterSlaveDataSourceIds)) {
            throw new IllegalArgumentException("ShardDataSource: [" + shardDataSourceContext.getShardDataSourceId()
                + "] masterSlaveDataSourceIds is empty");
        ...
        if (masterSlaveDataSourceIndex == -1) {
            throw new IllegalArgumentException("ShardDataSource: [" + shardDataSourceContext.getShardDataSourceId()
                + "], masterSlaveDataSourceId: [" + masterSlaveDataSourceId + "] is not in masterSlaveDataSourceIds: ["
                + masterSlaveDataSourceIds + "]");
        }
