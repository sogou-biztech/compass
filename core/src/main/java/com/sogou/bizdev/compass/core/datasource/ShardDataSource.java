...
public class ShardDataSource extends ComplexDataSource implements InitializingBean
{
	...
	public void afterPropertiesSet() throws Exception 
	{
		...
		{
			throw new IllegalArgumentException("masterSlaveDataSourceIds and idToMasterSlaveDataSource size not equal!");
		}
	...
	public Connection getConnection() throws SQLException 
	{
		...
		{
			logger.debug("get ShardProxyConnection from ShardDataSource:["+this.getId()+"], return:["+connection+"]");
		}
		return connection;
	}
	...
	public Connection getConnection(String username, String password)  throws SQLException 
	{
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("get ShardProxyConnection from ShardDataSource:["+this.getId()+"],username:["+username+"],password:["+password+"], return:["+connection+"]");
		}
		return connection;
	}
	...
	public List<DataSourceConnectionStatistic> getDataSourceConnectionStatistics()
	{
		...
		if(logger.isDebugEnabled())
		{
			logger.debug("get DataSource Connection Statistics from shardDataSource="+this.getId()+",result is ="+connectionStatistics);
		}
	 ...
	 * @return 如果用户返回的TableContexts列表为空或者某个TableConetx为null,会抛出异常
	 */
	...
	public void setMasterSlaveDataSources(List<MasterSlaveDataSource> masterSlaveDataSources)
	{
		...
		for(MasterSlaveDataSource masterSlaveDataSource : masterSlaveDataSources)
		{
			...
			{
				throw new IllegalArgumentException("shardDataSourceId:["+this.getId()+"],has masterSlaveDataSourceId null!");
			}
			if(this.idToMasterSlaveDataSource.containsKey(masterSlaveDataSourceId))
			{
				throw new IllegalArgumentException("shardDataSourceId:["+this.getId()+"],masterSlaveDataSourceId:["+masterSlaveDataSourceId+"] repeat!");
			}
	...
	public void setShardDataSourceRouteStrategy(ShardDataSourceRouteStrategy shardDataSourceRouterStrategy)
	{
		...
		if(this.shardDataSourceRouter.getShardDataSourceRouteStrategy()!=null)
		{
			throw new IllegalArgumentException("ShardDataSource:["+this.getId()+"],shardDataSourceRouterStrategy can not set repeat!");
		}
