...
public class MasterSlaveDataSource extends AbstractDataSource implements InitializingBean {
        ...
        for (Entry<String, SingleDataSource> entry : idToSlaveDataSource.entrySet()) {
            ...
            DataSourceAvailableChangeListener slaveListener = new DataSourceAvailableChangeListener() {
                ...
                public void availableChanged(String dataSourceId, Status status) {
                    ...
                    if (dataSource == null) {
                        logger.error("Invalid slave dataSource id: [{}] of MasterSlaveDataSource: [{}]",
                            dataSourceId, getId());
                        return;
                    }
                    ...
                    switch (status) {
                        case AVAILABLE:
                            ...
                            if (logger.isDebugEnabled()) {
                                logger.debug("aliveSlaveDataSources add datasource, dataSourceId={}, status={}",
                                    dataSourceId, DataSourceAvailableChangeListener.Status.AVAILABLE);
                            }
                            break;
                        case UNAVAILABLE:
                            aliveIdToSlaveDataSource.remove(dataSourceId);
                            if (logger.isDebugEnabled()) {
                                logger.debug("aliveSlaveDataSources remove datasource, dataSourceId={}, status={}",
                                    dataSourceId, DataSourceAvailableChangeListener.Status.UNAVAILABLE);
                            }
                            break;
                        default:
                    }
    ...
    public void removeUnavailableDataSource(String unavailableDataSourceId) {
        ...
        if (oldSlaveDataSource != null) {
            if (logger.isWarnEnabled()) {
                logger.warn("detect connection failed, aliveSlaveDataSources remove datasource, dataSourceId: [{}]",
                    unavailableDataSourceId);
            }
        }
    ...
    public Connection getConnection(String username, String password) throws SQLException {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("get MasterSlaveProxyConnection from MasterSlaveDataSource: [{}], "
                + "username: [{}], password: [{}]", this.getId(), username, password);
        }
        return connection;
    }
    ...
    public SingleDataSource select() {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("complete selection from MasterSlaveDataSource: [{}], shardDataSourceId: [{}]",
                this.getId(), shardDataSourceId);
        }
        return dataSource;
    }
    ...
    public List<DataSourceConnectionStatistic> getDataSourceConnectionStatistics() {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("get DataSource Connection Statistics from MasterSlaveDataSource: [{}], return {}",
    ...
    public void setSlaveDataSources(List<SingleDataSource> slaveDataSources) {
        ...
        for (SingleDataSource slaveDataSource : slaveDataSources) {
            ...
            if (!StringUtils.hasText(slaveDataSourceId)) {
                throw new IllegalArgumentException("masterSlaveDataSourceId: [" + this.getId() +
                    "] has slaveDataSourceId null");
            }

            if (this.idToSlaveDataSource.containsKey(slaveDataSourceId)) {
                throw new IllegalArgumentException("masterSlaveDataSourceId: [" + this.getId()
                    + "], slaveDataSourceId: [" + slaveDataSourceId + "] repeat");
            }
    ...
    public String toString() {
        return "MasterSlaveDataSource [id=" + this.getId()
            ...
            + ", aliveSlaveDataSources=" + aliveIdToSlaveDataSource
            + ", availabilityDetector=" + databaseAvailabilityDetector
            + ", pingStatement=" + pingStatement
            + ", masterSlaveDataSourceSelector=" + masterSlaveDataSourceSelector + "]";
    }
