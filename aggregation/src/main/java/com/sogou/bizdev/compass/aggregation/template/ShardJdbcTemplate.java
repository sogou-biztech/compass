 ...
 * @since 1.0.0
 */
public class ShardJdbcTemplate extends ShardJdbcConfig
    implements ShardJdbcOperations, InitializingBean, DisposableBean {
        ...
        if (rangedOperationParser.isRangedOperation(sql)) {
            ...
            for (Map.Entry<TableContext, Shard> entry : dataSourceEvidenceMap.entrySet()) {
                ...
                String replacedSql = sql
                    .replace(RANGE_PLACEHOLDER, RangedSqlUtil.buildPlaceHolder(groupedRouteKeys.size()));
     ...
     * @return
     */
    private ShardOperationProcessor<List<Map<String, Object>>> getAggregationProcessor(
        final AggregationDescriptor descriptor) {
        final List<QueryCallable<GroupListCollector>> callableList =
            new ArrayList<ShardJdbcTemplate.QueryCallable<GroupListCollector>>();

        ShardOperationProcessor<List<Map<String, Object>>> processor = new ShardOperationProcessor<List<Map<String,
            Object>>>() {
            @Override
            public void addOperation(Shard shard, String sql, Object[] args) {
                AggregationResultSetExtractor extractor = new AggregationResultSetExtractor(descriptor);
                QueryCallable<GroupListCollector> callable = new QueryCallable<GroupListCollector>(shard, sql, args,
                    extractor);
                callableList.add(callable);
            }
