...
public class RangedOperationParser {
    ...
    public SqlParameterList parseParameter(String sql, Object[] args) {
        String[] split = RangedSqlUtil.split(sql, ShardJdbcTemplate.RANGE_PLACEHOLDER);

        int leftCount = RangedSqlUtil.count(split[0], '?');
        int rightCount = RangedSqlUtil.count(split[1], '?');
